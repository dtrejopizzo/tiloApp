...
const SettingsPage: React.FC<SettingsPageProps> = ({ state, onUpdateUser }) => {
  const { user } = state
  ...
  useEffect(() => {
    ...
    if (w > 0 && h > 0 && a > 0) {
      ...
      // Suggest targets if not set
      if (!targetCalories) {
        setTargetCalories(Math.round(calculatedBmr * 1.2).toString()) // Sedentary factor
      }
      if (!targetWater) {
        setTargetWater(Math.round(w * 33).toString()) // 33ml per kg
      }
    } else {
      setBmi(null)
      setBmr(null)
  ...
  const handleUpdatePassword = (e: React.FormEvent) => {
    ...
    // In a real app, we would send this to a backend. For this demo, we simulate success.
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto pb-12">
      <header>
        <h2 className="text-3xl font-bold text-yaleblue">Settings & Profile</h2>
        <p className="text-muted-foreground">Personalize your targets, health metrics, and security.</p>
      </header>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Profile and Targets Column */}
        <div className="lg:col-span-2 space-y-8">
          ...
          {/* Security Section */}
          <div className="bg-card p-6 rounded-none shadow-sm border border-border">
            <h3 className="text-xl font-bold text-yaleblue mb-6">Security</h3>
          ...
          <div className="bg-card p-6 rounded-none shadow-sm border border-border">
            ...
            <button
              onClick={handleSave}
              className="w-full mt-6 py-2 text-xs font-bold text-cerulean hover:text-balticblue transition-colors flex items-center justify-center space-x-2"
            >
              <span>Apply Reminder Schedule</span>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="w-4 h-4"
                fill="none"
            ...
            <div className="space-y-6">
              <div className="p-5 bg-background rounded-none border border-border shadow-inner">
                ...
                <div className="mt-4 h-1.5 w-full bg-muted rounded-full overflow-hidden flex">
                  <div className="h-full bg-bondiblue" style={{ width: "18.5%" }} />
                  <div className="h-full bg-emeraldcustom" style={{ width: "25%" }} />
                  <div className="h-full bg-cerulean" style={{ width: "25%" }} />
                  <div className="h-full bg-yaleblue" style={{ width: "31.5%" }} />
                </div>
              </div>
              ...
              <div className="p-5 bg-background rounded-none border border-border shadow-inner">
                ...
                <div className="flex items-baseline space-x-1">
                  ...
                  <span className="text-xs font-black text-muted-foreground uppercase">kcal/day</span>
                </div>
                <p className="text-[10px] text-muted-foreground mt-2 font-medium italic">
                  Energy required for basic life functions at rest. Adding physical activity will increase your daily
                  expenditure (TDEE). Drink 250ml of water for every 30 mins of exercise.
                </p>
              </div>

              <div className="p-5 bg-limecream/10 rounded-none border border-limecream/20 relative overflow-hidden group">
                <div className="absolute -right-4 -top-4 w-16 h-16 bg-limecream/20 rounded-full group-hover:scale-110 transition-transform" />
                <h4 className="font-black text-yaleblue text-xs uppercase tracking-wider mb-2 flex items-center">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="w-4 h-4 mr-1 text-tropicalteal"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                ...
                <p className="text-xs text-muted-foreground leading-relaxed font-medium">
                  Your BMR accounts for 60-75% of your total calorie burn. Adding physical activity will increase your
                  daily expenditure (TDEE). Drink 250ml of water for every 30 mins of exercise.
                </p>
              </div>
            </div>
          </div>
